...
export default function TreatmentSlider({ treatments }: TreatmentSliderProps) {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash.startsWith("#service-")) {
        const serviceName = hash.replace("#service-", "").replace(/-/g, " ")
        ...
        if (serviceIndex !== -1 && containerRef.current) {
          ...
          containerRef.current.scrollTo({ left: cardWidth * serviceIndex, behavior: "smooth" })
        }
      }
    }
  ...
  const filteredTreatments = selectedFilter === "All" ? treatments : treatments.filter((t) => t.name === selectedFilter)
